 ...
 */
class MyIssueSampler : AbstractSampler() {
    companion object JMeterProperties {
        ...
        val uuid = Argument("UUID", "", "<meta>", "Account UUID")
        val amount = Argument("amount", "0", "<meta>", "The amount to issue.")
    }
    ...
    override fun setupTest(rpcProxy: CordaRPCOps, testContext: JavaSamplerContext) {
        ...
        // myIdentity = rpcProxy.nodeInfo().legalIdentities.first()!!

        accountUUID = UUID.fromString(testContext.getParameter(uuid.name, uuid.value)!!.toString())

        amountToIssue = testContext.getParameter(amount.name, amount.value)!!.toInt()
    ...
    override fun createFlowInvoke(rpcProxy: CordaRPCOps, testContext: JavaSamplerContext): FlowInvoke<*> {
        ...
        return FlowInvoke<IssueGSCoin>(IssueGSCoin::class.java, arrayOf(accountUUID, listOf(amountToIssue)))
    }
    ...
    override val additionalArgs: Set<Argument>
        get() = setOf(AbstractSampler.notary, uuid, amount)
